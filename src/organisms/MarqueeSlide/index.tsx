      ...
      <div className="flex flex-col gap-4 col-span-6 z-10">
        <div className="flex gap-2 items-center transition-colors duration-600 ease-in-out">
          <h2 className="text-md font-semibold">
            <a href={company.url}><span className="absolute block cursor-pointer -inset-x-4 -inset-y-4"/>
              <span className="flex items-center gap-2">
                {title} 
                <div className="size-1 rounded-full overflow-hidden bg-secondary-highlight"/> 
                {company.name}
              </span>
            </a>
          </h2>
